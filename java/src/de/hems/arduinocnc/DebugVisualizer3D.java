...
public class DebugVisualizer3D {
	...
	protected SimpleUniverse addSimulation(SimpleUniverse universe, ArrayList<Point3d> points) {
		...
		final double secPerSegment = 1;
		
		final float tracerRadius = 0.03f;
		
		final Color3f emissiveColor	= new Color3f(0.25f, 0.25f, 1f);
		final Color3f emptyColor	= new Color3f();
		/*
		final Color3f ambientColor	= new Color3f(1f, 0f, 0f);
		final Color3f emissiveColor	= new Color3f(0f, 1f, 0f);
		final Color3f diffuseColor	= new Color3f(0f, 0f, 1f);
		final Color3f specularColor	= new Color3f(1f, 1f, 1f);
		*/
		// -------- config end ---------
	    final long cycleTime = Math.round(num_points * secPerSegment * 100);
		...
		PositionPathInterpolator rotIntYear = new PositionPathInterpolator(
				new Alpha(-1, cycleTime), objYear, new Transform3D(), knots,
				keypoints);
		rotIntYear.setSchedulingBounds(bounds);
		
		objYear.addChild(tracer);
		objYear.addChild(rotIntYear);
		
		BranchGroup tracerGroup = new BranchGroup();
		tracerGroup.addChild(objYear);
