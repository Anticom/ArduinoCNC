...
* events.
*/
public class VisualizerExample extends JPanel implements ActionListener {
	 ...
	 * (directed-acyclic-graph) data structure called a scenegraph. The
	 * scenegraph tree has two major branches: the scene branch describes the
	 * objects and lights in the scene as well as any behaviors that act upon
	 * the scene. The view branch of the scenegraph describes the viewer of the
	 * scene, including their position, screen device, and viewing parameters.
	 * 
	 * SwingTest needs to modify the objects on the scene side of the scenegraph
	 ...
	 * behavior that will automatically modify the rotation components of a 4 ×
	 * 4 transformation matrix. Objects that are attached to the transformation
	 * matrix object (TransformGroup) will therefore be automatically rotated.
	 */
	...
	// and the scene.
	public VisualizerExample(ArrayList<Point3d> points) {
		this.points = points;
	...
	protected void init() {
		 ...
		 * Locales objects, each of which describes a discrete region within the
		 * scene, and has its own coordinate system. By deriving your own class
		 * from VirtualUniverse you can define utility methods or additional
		 * datastructures for your application.
		 */
		VirtualUniverse universe = createVirtualUniverse();

		/*
		 * A Locale object allows a coordinate system to be specified for a
		 * region within the scene. By having multiple Locales in your scene you
		 * can have multiple levels of detail without losing coordinate
		 * precision due to rounding errors.
		 */
		Locale locale = createLocale(universe);

		/*
		 * A BranchGroup is a branch of the scenegraph tree. A BranchGroup has a
		 * single parent Node and can have multiple child Nodes. The
		 ...
		 * background image, or background geometry for your application. In
		 * this example we simply create a light-gray background color and add
		 * it to the scene side of the scenegraph. Java 3D will automatically
		 * detect that it is a Background Node and paint the background color
		 * into the Canvas3D prior to rendering the scene geometry.
		 */
		 ...
		 * PhysicalBody that defines the characteristics of the viewer and their
		 * display hardware. A Canvas3D rendering component is attached to the
		 * View which is used to display the frames rendered.
		 */
		createView(vp);
	}

	/*
	 * Callback to allow the Canvas3D to be added to a Panel. This method is
	 * called by createView and allows the Canvas3D to be added to its parent
	 * GUI components, in this can as SwingTest is extends JPanel we can just
	 * add it directly to SwingTest.
	 */
	...
	protected View createView(ViewPlatform vp) {
		 ...
		 * 100.0 for the rear clipping plane.
		 */
	...
	protected Background createBackground() {
		...
		Background back = new Background(new Color3f(0, 0, 0));
	...
	/*
	 * Simple utility method that returns a bounding volume for the application.
	 * In this case we create a spherical volume, centered at 0,0,0 and with a
	 * radius of 100.
	 */
	...
	public TransformGroup[] getViewTransformGroupArray() {
		...
		/*
		 * Here we move the camera BACK a little so that we can see the origin
		 * (0,0,0). Note that we have to invert the matrix as we are moving the
		 * viewer not the scene.
		 */
	...
	protected BranchGroup createSceneBranchGroup() {
		...
		/*
		 * Create a TransformGroup to rotate the objects in the scene and set
		 * the capability bits on the TransformGroup so that it can be modified
		 * at runtime by the rotation behavior.
		 */
		...
		/*
		 * Create an Alpha interpolator to automatically generate modifications
		 * to the rotation component of the transformation matrix. This Alpha
		 * loops indefinitely and generates numbers from 0 to 1 every 4000
		 * milliseconds.
		 */
		 ...
		 * Here we will rotate from 0 to 2p degrees about the Y-axis based on
		 * the output of rotationAlpha.
		 */
		 ...
		 * and from the scenegraph. We store a reference to this subbranch of
	 ...
	 * BranchGroup is set so the BranchGroup can be identified later. User Data
	 * is a field that you can set on all Nodes in the scenegraph to allow you
	 * to associate your own data with particular scenegraph elements. The Cube
	 * must wrapped in a BranchGroup as only BranchGroups can be added and
	 * removed from the scenegraph at runtime— not Shape3Ds themselves which
	...
	protected void removeShape(String name) {
		try {
			...
			java.util.Enumeration enumeration = sceneBranchGroup
					.getAllChildren();
			int index = 0;

			// We then need to iterate through the Nodes to find the one with
			// the User Data that we would like to remove
			while (enumeration.hasMoreElements() != false) {
				SceneGraphObject sgObject = (SceneGraphObject) enumeration
						.nextElement();
	...
	protected void onSaveImage() {
		...
		try {
			...
			ImageIO.write(imageComponent.getImage(), "jpeg", fileOut);
	 ...
	 * This is not the way to implement menu handling for a large application
	 * (!) but it serves for our simple example.
	 */
	...
	/*
	 * Registers a window listener to handle ALT+F4 window closing. Otherwise
	 * the Swing application will just be made invisible when the parent frame
	 * is closed.
	 */
...
//		 * native. Our menu items will be dropped down in front of the Java 3D
//		 * Canvas3D so they must be created as heavyweight windows.
//		 */
//		JPopupMenu.setDefaultLightWeightPopupEnabled(false);
//
//		/*
//		 * Similarly we can declare that ToolTip windows are created as
//		 * heavyweight. Our application does not use tooltips. However, if a
//		 * toolbar was added the tooltips would overlap the Canvas3D and would
//		 * also need to be heavyweight windows.
//		 */
...
//
//		/*
//		 * Create a JMenuBar that will generate the events for the application.
//		 * We register the swingTest instance as a listener for the action
